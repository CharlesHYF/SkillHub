// 文件作用: Codex 适配器 —— 覆盖 Codex CLI 的探测(detect)与 MCP 实际态读取(read_state)。
...
/// Codex 适配器: 读取 `~/.codex/config.toml` 的 `[mcp_servers.*]` 表
...
impl AgentAdapter for CodexAdapter {
	...
	/// Codex 可托管 MCP 与 Skill(Skill 的落地读写留 Task 5/7, 能力矩阵上先声明支持)
	...
	/// 读取 `agent.config_path` 指向的 TOML 配置文件, 解析出 mcp_servers 表。配置文件不存在
	/// 视为"实际态为空"(工具装了但还没配任何 MCP, 不算错误); 文件存在但 TOML 解析失败才报错,
